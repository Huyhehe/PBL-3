...
export default {
  ...
  mutations: {
    ...
    ADD_NEW_EMP(state, data) {
      console.log(data);
      if (data == false) {
        state.addEmpSuccessful = false;
        return;
      }
      state.addEmpSuccessful = true;
    },
    ...
    SET_REGISTER_STATUS(state, status) {
      state.registerSuccessful = status;
    },
  ...
  getters: {
    ...
    getAddEmpSuccessful: (state) => state.addEmpSuccessful,
  },
  actions: {
    //USER
    ...
    async updateEmp({ commit }, user) {
      ...
      try {
        ...
        commit("SET_WARNING_MESSAGE", res.data.status);
      } catch (e) {
    ...
    async changePassword({ commit }, newIncoming) {
      ...
      try {
        ...
        commit("SET_WARNING_MESSAGE", res.data);
      } catch (e) {
        console.log(e);
        commit("SET_WARNING_MESSAGE", e.response.data);
      }
